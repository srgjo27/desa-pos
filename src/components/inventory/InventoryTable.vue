...
const { columns, sortedRows, toggleSort, getSortIcon } = useInventoryTable(rowsRef)
</script>

<template>
  <Table :columns="columns" :rows="sortedRows" :loading="loading" size="md" :striped="true" :bordered="true"
    :hoverable="true" emptyMessage="Tidak ada barang yang ditemukan.">
    ...
    <template v-for="col in columns" :key="col.key" #[`cell-${col.key}`]="{ value, row }">
      ...
      <span v-else-if="col.key === 'discount'">
        <span v-if="row.is_on_discount && row.discount_price"
      ...
      <span v-else-if="col.key === 'stock'">
        <span class="inline-flex items-center justify-center px-2.5 py-0.5 rounded-full font-bold text-xs border"
