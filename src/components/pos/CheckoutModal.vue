...
const grandTotal = computed(() => cartStore.totalAmount)
    ...
    <Dialog as="div" class="relative z-50" @close="handleClose">
      ...
      <div class="fixed inset-0 z-10 overflow-y-auto">
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
          ...
          >
            <DialogPanel class="relative transform overflow-hidden rounded-2xl bg-white text-left transition-all sm:my-8 sm:w-full sm:max-w-lg w-full">
              ...
              <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                ...
                <button @click="handleClose" class="text-gray-400 hover:text-gray-600 focus:outline-none p-1 bg-white rounded-md border border-gray-200">
              ...
              <div class="px-6 py-6 space-y-6">
                ...
                <div class="bg-blue-50 border border-blue-100 rounded-xl p-5 text-center">
                   <p class="text-sm font-semibold text-blue-600 mb-1">Total Tagihan ({{ cartStore.totalItems }} Item)</p>
                   <p class="text-4xl font-black text-blue-700 tracking-tight">{{ formatRupiah(grandTotal) }}</p>
                </div>

                <!-- Error Alert -->
                <div v-if="error" class="bg-red-50 border border-red-200 text-red-600 text-sm p-3 rounded-lg flex items-start gap-2">
                ...
                <div class="flex bg-gray-100 p-1 rounded-xl">
                  <button 
                    ...
                    :class="paymentMethod === 'CASH' ? 'bg-white text-gray-800' : 'text-gray-500 hover:text-gray-700'"
                  >Tunai (Cash)</button>
                  <button 
                    @click="paymentMethod = 'QRIS'"
                    class="flex-1 py-2 text-sm font-bold rounded-lg transition-colors focus:outline-none"
                    :class="paymentMethod === 'QRIS' ? 'bg-white text-gray-800' : 'text-gray-500 hover:text-gray-700'"
                  >QRIS</button>
                  <button 
                    @click="paymentMethod = 'TRANSFER'"
                    class="flex-1 py-2 text-sm font-bold rounded-lg transition-colors focus:outline-none"
                    :class="paymentMethod === 'TRANSFER' ? 'bg-white text-gray-800' : 'text-gray-500 hover:text-gray-700'"
                  >Transfer</button>
                </div>
                ...
                <div v-if="paymentMethod === 'CASH'" class="space-y-4 animate-in fade-in slide-in-from-top-2">
                  ...
                  <div class="flex flex-col gap-1.5">
                    ...
                    <div class="relative">
                      ...
                      <input 
                        :value="amountPaidRaw ? Number(amountPaidRaw).toLocaleString('id-ID') : ''"
                        @input="onCashInput"
                        type="text" 
                        inputmode="numeric"
                  ...
                  <div class="grid grid-cols-4 gap-2">
                    <button 
                      v-for="nom in quickNominals" :key="nom"
                  ...
                  >
                    ...
                    <span class="text-xl font-black font-mono tracking-tighter" :class="changeAmount >= 0 ? 'text-green-700' : 'text-red-700'">
                ...
                <div v-else class="text-center py-6 pb-2 animate-in fade-in zoom-in-95">
                  ...
                  <p class="text-sm text-gray-500 mt-2 px-8">Pastikan pelanggan telah melakukan transfer atau scan QRIS sesuai nominal tagihan <strong class="text-gray-700">{{ formatRupiah(grandTotal) }}</strong> sebelum memproses.</p>
                </div>
