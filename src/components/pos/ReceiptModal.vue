...
import { generateReceiptPDF } from '@/services/pdfExportService'
...
async function handleExportReceiptPDF() {
  if (!props.receiptData || !props.receiptData.items) return
  isExportingPDF.value = true

  try {
    await generateReceiptPDF({
      saleId: props.receiptData.saleId,
      date: props.receiptData.createdAt,
      cashierName: props.receiptData.cashierName,
      items: props.receiptData.items,
      subtotal: props.receiptData.subtotalBeforeDiscount || props.receiptData.totalAmount || 0,
      itemDiscounts: props.receiptData.totalItemDiscounts || 0,
      transactionDiscount: props.receiptData.discountAmount || 0,
      total: props.receiptData.grandTotal || 0,
      amountPaid: props.receiptData.amountPaid || 0,
      change: props.receiptData.changeAmount || 0,
      paymentMethod: props.receiptData.paymentMethod || 'CASH'
    })
  } catch (err) {
    console.error('Error exporting receipt PDF:', err)
    alert('Gagal membuat nota PDF. Pastikan jsPDF sudah terinstall.')
  } finally {
            ...
            <DialogPanel
              ...
              <div class="bg-gray-50 p-4 border-t border-gray-200 flex flex-col gap-2 print:hidden">
                ...
                <div class="grid grid-cols-3 gap-2 mb-2">
                  ...
                  <button @click="handleExportReceiptPDF" :disabled="isExportingPDF"
                    class="flex items-center justify-center gap-2 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg text-sm font-bold hover:bg-gray-50 transition-colors focus:outline-none disabled:opacity-50">
