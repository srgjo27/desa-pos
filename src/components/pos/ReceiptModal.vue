      ...
      <div class="fixed inset-0 z-10 overflow-y-auto print:static print:overflow-visible">
        <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0 print:p-0">
          <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
            ...
            <DialogPanel class="relative transform overflow-hidden rounded-2xl bg-white text-left transition-all sm:my-8 w-full max-w-sm print:transform-none print:w-full print:max-w-none print:m-0">
              
              <!-- Struk Container (Yang Akan Di-Print / Capture) -->
              <div 
                ref="receiptRef" 
                class="bg-white p-6 md:p-8 font-mono text-sm text-gray-800"
              ...
              >
                ...
                <div class="space-y-1 text-xs mb-6">
                  <div class="flex justify-between text-gray-500">
                    <span>Subtotal</span>
                    <span>{{ formatRupiah(props.receiptData.totalAmount || 0) }}</span>
                  </div>
                  <div v-if="props.receiptData.discountAmount > 0" class="flex justify-between text-gray-500">
                    <span>Diskon</span>
              ...
              <div class="bg-gray-50 p-4 border-t border-gray-200 flex flex-col gap-2 print:hidden">
                 ...
                 <button 
                  @click="handleClose"
                  class="w-full text-center py-3 bg-white border-2 border-blue-600 text-blue-600 rounded-lg font-bold text-sm hover:bg-blue-50 transition-colors focus:outline-none"
                 >Tutup Salinan & Lanjut Jualan</button>
              </div>
