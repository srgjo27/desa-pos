...
<template>
  <div class="h-screen flex overflow-hidden bg-gray-50 font-sans">
    ...
    <div 
      class="flex-1 flex flex-col h-full bg-gray-50 overflow-hidden"
      :class="{ 'hidden lg:flex': showMobileCart }"
    >
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="flex flex-col">
            ...
            <div class="flex items-center gap-2 mt-1">
              ...
              <span 
                v-else 
                class="px-2 py-0.5 bg-amber-100 text-amber-700 text-xs font-bold rounded"
              >
                Tutup Shift
              </span>
            </div>
      ...
      <div class="p-4 bg-white border-b border-gray-200 flex items-center">
        <div class="relative w-full">
          ...
          <input 
            ...
            class="w-full bg-gray-100 border border-transparent focus:bg-white focus:border-blue-500 rounded-lg pl-12 pr-4 py-3 text-sm font-medium text-gray-800 placeholder-gray-500 focus:outline-none transition-colors" 
          />
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-4 custom-scrollbar bg-grid-pattern">
        ...
        <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          <div 
            ...
            class="bg-white border border-gray-200 rounded-lg overflow-hidden hover:border-blue-500 cursor-pointer flex flex-col h-full transition-colors active:bg-blue-50"
          >
             ...
             <div class="p-4 flex flex-col flex-1">
                ...
                <div class="flex items-end justify-between mt-4">
                   <p class="font-bold text-blue-600 text-sm">{{ formatRupiah(product.price) }}</p>
      ...
      <div 
        class="lg:hidden bg-white border-t border-gray-200 p-4 shrink-0"
        v-if="cartStore.totalItems > 0"
      >
        <button 
          @click="showMobileCart = true" 
          class="w-full bg-blue-600 text-white p-4 rounded-lg font-bold flex items-center justify-between focus:outline-none"
        >
    ...
    <div 
      class="w-full lg:w-96 bg-white border-l border-gray-200 h-full z-20"
    ...
    >
       ...
       <div class="p-4 border-b border-gray-200 flex items-center justify-between bg-white shrink-0">
         <div class="flex items-center gap-4">
           ...
           <div>
             ...
             <p class="text-xs text-blue-600 font-bold" v-if="cartStore.totalItems > 0">{{ cartStore.totalItems }} Item</p>
           </div>
         </div>
         <button 
       ...
       <div class="flex-1 p-4 bg-gray-50 overflow-y-auto custom-scrollbar flex flex-col gap-4">
          ...
          <div v-if="cartStore.items.length === 0" class="flex-1 flex flex-col items-center justify-center text-gray-400">
             <svg class="w-16 h-16 mb-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>
          ...
          <template v-else>
            ...
            >
               ...
               <div class="pr-8">
                  ...
                  <p class="text-blue-600 font-bold text-sm mt-1">{{ formatRupiah(item.price) }}</p>
               </div>

               <!-- Qty Controller -->
               <div class="flex items-center justify-between border-t border-gray-100 pt-4 mt-auto">
                 <p class="text-xs font-bold text-gray-500">Sub: <span class="text-gray-800">{{ formatRupiah(item.price * item.qty) }}</span></p>
                 <div class="flex items-center gap-4 bg-gray-100 p-1 rounded-lg">
                   <button 
                     @click="cartStore.removeItem(item.product_id)" 
                     class="w-8 h-8 flex items-center justify-center bg-white text-gray-600 hover:text-blue-600 rounded font-bold shadow-sm focus:outline-none"
                   >-</button>
                   <span class="w-4 text-center text-sm font-bold text-gray-800">{{ item.qty }}</span>
                   <button 
                     @click="cartStore.addItem({id: item.product_id, name: item.name, price: item.price})" 
                     class="w-8 h-8 flex items-center justify-center bg-white text-gray-600 hover:text-blue-600 rounded font-bold shadow-sm focus:outline-none"
                   >+</button>
                 </div>
               </div>
       ...
       <div class="p-4 bg-white border-t border-gray-200 shrink-0">
          <div class="flex flex-col gap-2 mb-4">
             ...
             <div class="flex items-center justify-between text-sm text-gray-600 font-medium pb-4 border-b border-gray-200">
                <span>Diskon</span>
                <span class="text-green-600">- Rp0</span>
             </div>
             <div class="flex items-center justify-between text-lg font-bold text-gray-800 pt-2">
                <span>Total</span>
                <span class="text-blue-600">{{ formatRupiah(cartStore.totalAmount) }}</span>
             </div>
          </div>
          ...
          <button 
            ...
            class="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 text-white p-4 rounded-lg font-bold text-sm transition-colors focus:outline-none flex justify-center items-center gap-2"
          >
             Checkout 
