  ...
  <AdminLayout title="Analisis Keuangan" subtitle="Laporan Laba/Rugi BUMDes" activeTab="analytics">
      ...
      <div class="bg-white p-4 border border-gray-200 rounded-lg flex flex-col sm:flex-row items-end gap-4 mb-6">
         <div class="w-full sm:w-auto flex-1 max-w-xs">
           ...
           <input v-model="startDate" type="date" class="w-full bg-gray-50 border border-gray-300 rounded-md px-3 py-2 text-sm focus:border-green-600 outline-none font-medium transition-colors">
         </div>
         <div class="w-full sm:w-auto flex-1 max-w-xs">
           <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Sampai Tanggal</label>
           <input v-model="endDate" type="date" class="w-full bg-gray-50 border border-gray-300 rounded-md px-3 py-2 text-sm focus:border-green-600 outline-none font-medium transition-colors">
         </div>
         <button 
            @click="fetchAnalytics"
            :disabled="analytics.loading.value"
            class="w-full sm:w-auto px-5 py-2 bg-gray-900 hover:bg-black text-white rounded-md font-bold text-sm transition-colors focus:outline-none flex items-center justify-center gap-2 border border-transparent disabled:opacity-50"
         >
           <svg v-if="analytics.loading.value" class="animate-spin w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <div class="bg-white p-5 border border-gray-200 rounded-lg flex items-center gap-4 relative overflow-hidden">
           ...
           <div class="w-12 h-12 rounded-full bg-green-100 text-green-700 flex items-center justify-center border border-green-200 z-10">
      ...
      <div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
         <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            ...
            <span class="text-xs font-bold bg-white border border-gray-200 px-2 py-1 rounded text-gray-500">{{ analytics.profitData.value.length }} Transaksi</span>
         </div>
           ...
           <table class="w-full text-left text-sm text-gray-700">
             <thead class="bg-white text-gray-500 border-b border-gray-200 text-xs uppercase tracking-wider font-bold">
               <tr>
                 ...
                 <th class="px-6 py-4 text-right border-l border-gray-100" title="Total Nilai Barang (HPP)">Total Modal</th>
             ...
             <tbody class="divide-y divide-gray-100">
               ...
               <tr v-else-if="analytics.profitData.value.length === 0">
                  <td colspan="5" class="px-6 py-12 text-center text-gray-400 font-medium">Tidak ada transaksi ditemukan pada rentang tanggal ini.</td>
               </tr>
               <tr 
                 v-else
                 v-for="item in analytics.profitData.value" 
                 :key="item.sale_id"
                 class="hover:bg-gray-50/80 transition-colors group"
               >
                 <td class="px-6 py-3 border-r border-gray-100">
                    <p class="font-bold text-gray-900">{{ new Date(item.created_at).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' }) }}</p>
                    <p class="text-xs text-gray-400 font-mono">{{ new Date(item.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) }}</p>
                 </td>
                 <td class="px-6 py-3 border-r border-gray-100 font-medium text-gray-700">
                   <div class="flex items-center gap-2">
                     <div class="w-6 h-6 rounded bg-gray-100 flex items-center justify-center text-xs font-bold text-gray-400">
                       {{ item.kasir.charAt(0).toUpperCase() }}
                     </div>
                     {{ item.kasir }}
                   </div>
                 </td>
                 <td class="px-6 py-3 text-right font-mono text-sm text-gray-500 border-l border-gray-100">{{ formatRupiah(item.total_modal || 0) }}</td>
                 <td class="px-6 py-3 text-right font-mono text-sm font-bold text-gray-800 border-l border-gray-100">{{ formatRupiah(item.grand_total || 0) }}</td>
