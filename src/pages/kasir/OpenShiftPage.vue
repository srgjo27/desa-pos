...
<template>
  <div class="min-h-screen bg-linear-to-br from-green-50 via-white to-indigo-50 flex items-center justify-center p-4">

    <!-- Card -->
    <div class="w-full max-w-md bg-white rounded-xl border border-gray-100 overflow-hidden">

      <!-- Header biru -->
      <div class="bg-linear-to-r from-green-600 to-cyan-600 px-8 py-6 text-white text-center relative overflow-hidden">
        ...
        <p class="text-green-100 text-sm mt-1 relative z-10">{{ formatDateLong(now) }}</p>
      </div>
      ...
      <div class="px-8 py-6 space-y-5">
        ...
        <div class="flex items-center gap-3 bg-green-50 border border-green-100 rounded-xl px-4 py-3">
          <div
            class="w-10 h-10 rounded-full bg-green-600 flex items-center justify-center text-white font-bold text-sm shrink-0">
            {{ authStore.user?.name?.charAt(0).toUpperCase() }}
          </div>
          <div>
            <p class="text-xs text-gray-500 font-medium">Selamat datang,</p>
            <p class="font-bold leading-tight">{{ authStore.user?.name }}</p>
        ...
        <!-- Judul -->
        <div>
          <h1 class="text-xl font-bold">Buka Shift Hari Ini</h1>
        ...
        <form id="open-shift-form" @submit.prevent="handleSubmit" class="space-y-4" novalidate>
          ...
          <!-- Error global -->
          <div v-if="error"
            class="flex items-start gap-2 bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-700"
            role="alert">
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <label for="opening-cash" class="text-sm font-semibold">
              Uang Modal di Laci Kasir
            </label>

            <div class="relative">
              <span
                class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-semibold text-sm select-none">Rp</span>
              <input id="opening-cash" type="text" inputmode="numeric" placeholder="0" maxlength="15" :class="[
            ...
            <p v-if="form.openingCash" class="text-xs text-green-600 font-medium">
              {{ formatRupiah(Number(form.openingCash)) }}
            </p>
          </div>

          <!-- Tombol mulai shift -->
          <BaseButton id="start-shift-btn" type="submit" :loading="loading" :disabled="loading" full-width>
