...
<template>
  <AdminLayout title="Rekap Shift Kasir" subtitle="Analisis absensi dan durasi kerja harian" activeTab="shifts">
    ...
    <div class="mb-6 flex items-center justify-between gap-4">
      <div>
        <h2 class="text-lg font-bold text-gray-900">Riwayat Absensi</h2>
        <p class="text-sm text-gray-500">Memonitor kedisiplinan shift kerja karyawan berdasarkan waktu standar 8 Jam.</p>
      </div>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            >
              <td class="px-6 py-4 border-r border-gray-100">
                <div class="font-bold text-gray-900">{{ shift.users?.name || 'GUEST' }}</div>
              ...
              <td class="px-6 py-4 text-center border-r border-gray-100">
                ...
                <div class="text-xs font-bold text-gray-500 bg-gray-100 px-2 py-1 rounded inline-block">
                  ...
                  - 
                  {{ shift.closed_at ? new Date(shift.closed_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) : 'Sekarang' }}
                </div>
              </td>
      ...
      <!-- Pagination Footer -->
      <div v-if="shifts.length > 0" class="px-6 py-4 border-t border-gray-200 bg-gray-50 flex flex-col sm:flex-row items-center justify-between gap-4">
